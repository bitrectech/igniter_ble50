...
void (*write_event_monitors)(esp_ble_gatts_cb_param_t *param);
...
static void gap_event_handler(esp_gap_ble_cb_event_t event,
    ...
    case ESP_GAP_BLE_EXT_ADV_SET_PARAMS_COMPLETE_EVT:
        ESP_LOGI(GATTS_TABLE_TAG,
                 "ESP_GAP_BLE_EXT_ADV_SET_PARAMS_COMPLETE_EVT status %d",
                 param->ext_adv_set_params.status);
        esp_ble_gap_config_ext_adv_data_raw(
            EXT_ADV_HANDLE, sizeof(ext_adv_raw_data), &ext_adv_raw_data[0]);
        break;
    case ESP_GAP_BLE_EXT_ADV_DATA_SET_COMPLETE_EVT:
        ESP_LOGI(GATTS_TABLE_TAG,
                 "ESP_GAP_BLE_EXT_ADV_DATA_SET_COMPLETE_EVT status %d",
                 param->ext_adv_data_set.status);
        esp_ble_gap_ext_adv_start(NUM_EXT_ADV_SET, &ext_adv[0]);
        break;
    case ESP_GAP_BLE_EXT_ADV_START_COMPLETE_EVT:
        ESP_LOGI(GATTS_TABLE_TAG,
                 "ESP_GAP_BLE_EXT_ADV_START_COMPLETE_EVT, status = %d",
                 param->ext_adv_data_set.status);
        break;
    case ESP_GAP_BLE_ADV_TERMINATED_EVT:
        ESP_LOGI(GATTS_TABLE_TAG, "ESP_GAP_BLE_ADV_TERMINATED_EVT, status = %d",
                 param->adv_terminate.status);
        if (param->adv_terminate.status == 0x00) {
            ESP_LOGI(GATTS_TABLE_TAG,
                     "ADV successfully ended with a connection being created");
        }
        break;
    case ESP_GAP_BLE_PASSKEY_REQ_EVT: /* passkey request event */
        /* Call the following function to input the passkey which is displayed
    ...
    case ESP_GAP_BLE_SEC_REQ_EVT:
        ...
        If not accept the security request, should send the security response
        with negative(false) accept value*/
    ...
    case ESP_GAP_BLE_PASSKEY_NOTIF_EVT: /// the app will receive this evt when
        ...
        ESP_LOGI(GATTS_TABLE_TAG, "The passkey Notify number:%06d",
                 param->ble_security.key_notif.passkey);
        break;
    ...
    case ESP_GAP_BLE_SET_LOCAL_PRIVACY_COMPLETE_EVT:
        ESP_LOGI(GATTS_TABLE_TAG,
                 "ESP_GAP_BLE_SET_LOCAL_PRIVACY_COMPLETE_EVT, tatus = %x",
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event,
    ...
    case ESP_GATTS_REG_EVT:
        ...
        esp_ble_gatts_create_attr_tab(gatt_db, gatts_if, HRS_IDX_NB,
                                      SVC_INST_ID);
        break;
    ...
    case ESP_GATTS_CREAT_ATTR_TAB_EVT: {
        ...
        if (param->create.status == ESP_GATT_OK) {
            ...
            ESP_LOGE(GATTS_TABLE_TAG,
                     " Create attribute table failed, error code = %x",
                     param->create.status);
        }
        break;
    }
...
void set_lifecycle(void (*write_event_monitor)(esp_ble_gatts_cb_param_t)){
    vTaskDelay(100);
    write_event_monitors = (*write_event_monitor);
